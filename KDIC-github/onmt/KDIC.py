...
from onmt.VariationalGraphBert import SingleleHypEncoder


class MediatorExtractor(nn.Module):
    def __init__(self, config):
        super(MediatorExtractor, self).__init__()
...
class TransformerEncoder(nn.Module):
    def __init__(self, config, keep_multihead_output=False):
        ...
        self.graph_extractor = MediatorExtractor(config)            
...
class KDIC(BertPreTrainedModel):

    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(KDIC, self).__init__(config)
        self.config = config
        ...
        self.compare_attn = nn.MultiheadAttention(1024, num_heads=1, batch_first=True)
        # config.merge_layer=6
        self.single_sentence_model = SingleleHypEncoder(config)
        self.cls_reduce = BertOnlyNSPHead(config)
    ...
    def forward(self, input_ids_p, input_ids_q=None, attn_mask_p = None, sentence_inds_p=None, attn_mask_q=None, sentence_inds_q=None, token_type_ids=None, graph=None, 
                      is_train=True, output_all_encoded_layers=False, head_mask=None, compared_sample=None, hyp_ids=None, hyp_mask=None):
        hyp_embedding_output = self.embeddings(hyp_ids)
        single_rep = self.single_sentence_model(hyp_embedding_output, hyp_mask)
        ...
        if compared_sample:
            com_input_ids, com_input_masks, com_sentence_inds, com_ans = compared_sample[0], compared_sample[1], compared_sample[2], compared_sample[3]
            com_token_type_ids = torch.zeros_like(com_input_ids)
            cel=[]
            for n in range(2):
                com_embedding_output = self.embeddings(com_input_ids[:,n,:], com_token_type_ids[:,n,:])
                comp_encoded_layers, _ =self.encoder(hidden_states=com_embedding_output, attention_mask=com_input_masks[:,n,:],
                             sentence_ind=com_sentence_inds[:,n,:],
                             output_all_encoded_layers=output_all_encoded_layers,
                             head_mask=head_mask)
                com_decoded_layers, _ = self.decoder(comp_encoded_layers[-1], graph_vectors, attention_mask=None,
                                                   attn_scores=attn_scores_p)
                cel.append(self.pooler(com_decoded_layers[-1]))
            compared_rep = torch.stack(cel, dim=1)
        ...
        if compared_sample:
            _, attn_weights = self.compare_attn(pooled_output.unsqueeze(1), compared_rep, compared_rep)
            compare_attscores = attn_weights
            # attn_output_weights = torch.bmm(pooled_output.unsqueeze(1), compared_rep.transpose(1, 2))
            # compare_attscores = attn_output_weights / math.sqrt(compared_rep.shape[-1])

        cls_scores = self.cls(pooled_output-0.1*single_rep)
        # cls_scores = self.cls(pooled_output)
        # red_scores = self.cls_reduce(single_rep)
        ...
        elif compared_sample:
            return cls_scores, attn_scores_p, pooled_output, compare_attscores
        else:
            return cls_scores, attn_scores_p, pooled_output
