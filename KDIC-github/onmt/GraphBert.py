...
class GATResMergerLayer(nn.Module):
    def __init__(self, config):
        ...
        neg_layer = GATLayer(config)
        self.neg_layer = neg_layer
        self.neg_layer.output_attentions = True
    ...
    def forward(self, context_vectors, graph_vectors, sent_ind):
        ...
        context_vectors_negative_updated = self.neg_layer(context_vectors, -graph_vectors, sent_ind, drop_first_token=False)

        if self.config.layer_norm:
            context_vectors_updated = BertLayerNorm(context_vectors + context_vectors_updated + context_vectors_negative_updated)
        else:
            context_vectors_updated = context_vectors + context_vectors_updated + context_vectors_negative_updated
